            ...
            {actors.map((actor) => (
              <div key={actor.id} className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => { setVoiceEditId(voiceEditId === actor.id ? null : actor.id); setVoiceFile(null); }}
                    title={actor.voiceRefBucket ? "Replace voice sample" : "No voice yet — add one to enable cloning"}
                    className={`flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] font-bold ${
                      actor.voiceRefBucket
                        ? "border-emerald-400/25 bg-emerald-500/10 text-emerald-200"
                        : "border-amber-400/25 bg-amber-500/10 text-amber-200"
                    }`}
                  >
                    <Mic size={11} />
                    {actor.voiceRefBucket ? "Voice cloned" : "Add voice"}
                  </button>
                </div>
